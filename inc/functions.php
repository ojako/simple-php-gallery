...
//print image wall $target=target dir, $count=number of files to load
function imageWall($target){
	//create ul
	echo '<ul class="g">'."\r\n";
	//open media folder
	if ($handle = opendir('media')) {
		//check for $target dir else return err
		while (false !== ($entry = readdir($handle))) {
			...
			if($entry == $target){
				for ($i=0; $i<count($files); $i++){
					...
					//check full image file exists before printing (PRINT WITH AJAX LAZY)
					if(file_exists('media/'.$entry.'/'.$filename)){
						echo '<li><a href="media/'.$entry.'/'.$filename.'" class="image-hover lb" data-lightbox-hidpi="image_large@2x.jpg" data-lightbox-type="ajax" data-lightbox-gallery="'.$entry.'"><img src="'.$num.'" alt="'.$entry.' - '.$num.'"/></a>'."</li>\r\n";
					}
				}
			}
		}
	closedir($handle);
	echo "</ul>\r\n";
	//if media dir cannot be found return err
	}else{
		echo "<p>Gallery cannot open directory</p>";
	}
}

//strip unusual characters and file extentions from strings (used to clean filenames for use as alt tags and titles)
...
//below is old and unused, for reference only
